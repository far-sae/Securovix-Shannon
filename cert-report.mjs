#!/usr/bin/env node
/**
 * Automated security evidence report generator. Turns confirmed findings into a reviewable report
 * (Markdown + self-contained HTML) with references to recognized methodology:
 *   - CVSS v3.1 score + vector per finding (via cvss.mjs)
 *   - OWASP WSTG (test IDs) + OWASP ASVS (requirements) mapping  [methodology coverage]
 *   - OWASP Top 10 / CWE / MITRE ATT&CK + PCI DSS / ISO 27001 reference mappings
 *   - Evidence + integrity note, scope/authorization statement, limitations, and a SIGN-OFF block
 *
 * The output is not an audit, attestation, certification, or guarantee of compliance.
 */
...
export function buildCertReport(
  ...
  // ---------- Markdown ----------
  const md = [
    '# Automated Security Evidence Report',
    '',
    ...
    '',
    '## 2. Methodology',
    'Testing references OWASP Web Security Testing Guide (WSTG v4.2), OWASP ASVS v4.0, and NIST SP 800-115. Applicable confirmed findings include a benign proof marker; unverified observations are kept separate. Framework references are informational and do not establish compliance.',
    '',
    ...
    '| Reviewer credentials (if applicable) | __________________________ |',
    '| Reviewed & validated | ☐ |',
    '| Signature | __________________________ |',
    '| Date | __________________________ |',
    '',
    `_Automated findings above were generated by ${engine}. This report is not an independent penetration test, audit, attestation, certification, or guarantee of compliance. Qualified human review is recommended._`,
  ].join('\n');
...
<h1>Automated Security Evidence Report</h1>
...
<p class="meta">Surface: ${esc(crawl)}. References: OWASP WSTG v4.2, OWASP ASVS v4.0, NIST SP 800-115. Applicable confirmed findings include a benign proof marker; unverified observations are kept separate. Classes executed (${classesTested.length}): ${esc(classesTested.join(', ') || '—')}.</p>
<h2>Findings</h2>
...
<h2>Informational framework mappings</h2>
<p class="meta">Per-finding references to OWASP Top 10 (2021), CWE, MITRE ATT&amp;CK, PCI DSS 4.0, and ISO 27001:2022 are included above. They do not establish certification or compliance. A clean result covers only the tested classes.</p>
<h2>Sign-off</h2>
<table class="signoff"><tbody>
<tr><th>Tester (name)</th><td></td></tr>
<tr><th>Reviewer credentials (if applicable)</th><td></td></tr>
<tr><th>Reviewed &amp; validated</th><td>☐</td></tr>
<tr><th>Signature</th><td></td></tr>
<tr><th>Date</th><td></td></tr>
</tbody></table>
<p class="disc">Automated findings generated by ${esc(engine)}. This report is not an independent penetration test, audit, attestation, certification, or guarantee of compliance. Automated coverage (${classesTested.length} classes) is not a substitute for qualified human testing.</p>
</body></html>`;
